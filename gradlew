...
if ! "$JAVACMD" -version >/dev/null 2>&1 ; then
    die "ERROR: Unable to execute Java.
...
if $cygwin || $msys ; then
    ...
    # Convert application args to Windows paths.
    for arg do
        if
            case $arg in
              -*) false ;;
...
# Collect all arguments for the Java process.
set -- \
...
# Use maximum JVM options when available.
exec "$JAVACMD" "$@" 
